...
- **MySQL数据库支持**：可选的数据库功能，实现搜索结果持久化存储，优先从数据库查询，提升响应速度。详情参考[**数据库配置说明**](docs/database.md)  
...
| **DB_ENABLED** | 是否启用数据库功能 | `false` | 设置为`true`启用数据库，详见[数据库配置](docs/database.md) |
| **DB_HOST** | 数据库主机地址 | `localhost` | MySQL服务器地址 |
| **DB_PORT** | 数据库端口 | `3306` | MySQL端口 |
| **DB_USER** | 数据库用户名 | `root` | 数据库登录用户 |
| **DB_PASSWORD** | 数据库密码 | 无 | 数据库登录密码（必需） |
| **DB_NAME** | 数据库名称 | `pansou` | 使用的数据库名 |
