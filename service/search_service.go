...
func NewSearchService(pluginManager *plugin.PluginManager) *SearchService {
	...
	return &SearchService{
		...
		searchDAO:     database.NewSearchDAO(),
	}
}
...
func (s *SearchService) Search(keyword string, channels []string, concurrency int, forceRefresh bool, resultType string, sourceType string, plugins []string, cloudTypes []string, ext map[string]interface{}) (model.SearchResponse, error) {
	...
	// 第一步：尝试从数据库搜索
	var dbResults []model.SearchResult
	var dbErr error
	if database.IsEnabled() && !forceRefresh {
		dbResults, dbErr = s.searchDAO.SearchByKeyword(keyword)
		if dbErr != nil {
			fmt.Printf("⚠️ 数据库搜索失败: %v，将使用插件搜索\n", dbErr)
		} else if len(dbResults) > 0 {
			fmt.Printf("✅ 从数据库找到 %d 条结果\n", len(dbResults))
			// 数据库有结果，直接返回
		...
		fmt.Printf("ℹ️ 数据库中没有找到结果，将使用插件搜索\n")
	}

	// 第二步：数据库没有结果，并行获取TG搜索和插件搜索结果
	...
	// 第三步：将搜索结果保存到数据库（异步执行）
	if database.IsEnabled() && len(allResults) > 0 {
		go func(kw string, results []model.SearchResult) {
			if err := s.searchDAO.SaveSearchResults(kw, results); err != nil {
				fmt.Printf("⚠️ 保存搜索结果到数据库失败: %v\n", err)
			} else {
				fmt.Printf("✅ 成功保存 %d 条结果到数据库\n", len(results))
			}
		}(keyword, allResults)
	}
