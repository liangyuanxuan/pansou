...
func init() {
	...
	// 初始化ES缓存服务
	if config.AppConfig != nil && config.AppConfig.ESEnabled {
...
func (s *SearchService) Search(keyword string, channels []string, concurrency int, forceRefresh bool, resultType string, sourceType string, plugins []string, cloudTypes []string, ext map[string]interface{}) (model.SearchResponse, error) {
	...
	// 步骤1: 优先从本地MySQL数据库搜索（如果未强制刷新）
	var localResults []model.SearchResult
	if !forceRefresh && s.adminService != nil {
		localData, err := s.adminService.SearchLocalData(keyword)
		if err == nil && len(localData) > 0 {
			fmt.Printf("✅ [本地MySQL] 找到 %d 条数据，关键词: %s\n", len(localData), keyword)
			localResults = localData
		} else if err != nil {
			fmt.Printf("⚠️ [本地MySQL] 查询失败: %v\n", err)
		}
	}

	// 步骤2: 如果本地没有数据或强制刷新，执行在线搜索
	var tgResults []model.SearchResult
	var pluginResults []model.SearchResult
	var needOnlineSearch bool

	if forceRefresh || len(localResults) == 0 {
	...
	if len(localResults) > 0 {
		...
		fmt.Printf("📊 [结果合并] 本地数据: %d 条\n", len(localResults))
	}
	if needOnlineSearch {
		onlineResults := mergeSearchResults(tgResults, pluginResults)
		allResults = mergeSearchResults(allResults, onlineResults)
		fmt.Printf("📊 [结果合并] 在线搜索: %d 条 (TG: %d, Plugin: %d)\n",
			len(onlineResults), len(tgResults), len(pluginResults))

		// 步骤4: 异步保存新搜索结果到MySQL和ES
		if len(onlineResults) > 0 && s.adminService != nil {
			go func(results []model.SearchResult, kw string) {
				// 确定来源
				source := "mixed"
				if sourceType == "tg" {
					source = "tg"
				...
				if err := s.adminService.SaveSearchResults(kw, source, results); err != nil {
					fmt.Printf("⚠️ [自动保存] 保存搜索结果失败: %v\n", err)
				} else {
					fmt.Printf("💾 [自动保存] 搜索结果已保存: 关键词=%s, 数量=%d\n", kw, len(results))
				}
			}(onlineResults, keyword)
		}
	}
...
func (s *SearchService) searchTG(keyword string, channels []string, forceRefresh bool) ([]model.SearchResult, error) {
	...
	if !forceRefresh {
		// 1. 优先从ES获取
		if esCacheService != nil && elasticsearch.IsEnabled() {
			results, hit, err := esCacheService.Get(cacheKey)
			if err == nil && hit {
				fmt.Printf("✅ [TG:%s] ES缓存命中 | 结果数: %d\n", keyword, len(results))
				// 记录搜索历史
				go esCacheService.RecordSearchHistory(keyword)
				return results, nil
			}
		}
			...
			if enhancedTwoLevelCache != nil {
				...
				if err == nil && hit {
					...
					if err := enhancedTwoLevelCache.GetSerializer().Deserialize(data, &results); err == nil {
						fmt.Printf("✅ [TG:%s] 本地缓存命中 | 结果数: %d\n", keyword, len(results))
						// 异步同步到ES
						if esCacheService != nil && elasticsearch.IsEnabled() {
							go esCacheService.Set(cacheKey, keyword, channels, nil, "tg", results)
						}
						return results, nil
					}
				}
	...
	if len(results) > 0 {
		...
		if esCacheService != nil && elasticsearch.IsEnabled() {
			go func(res []model.SearchResult) {
				if err := esCacheService.Set(cacheKey, keyword, channels, nil, "tg", res); err != nil {
					fmt.Printf("❌ [TG:%s] ES缓存写入失败: %v\n", keyword, err)
				} else {
					fmt.Printf("💾 [TG:%s] ES缓存写入成功 | 结果数: %d\n", keyword, len(res))
				}
				// 记录搜索历史
				esCacheService.RecordSearchHistory(keyword)
			}(results)
		}
...
func (s *SearchService) searchPlugins(keyword string, plugins []string, forceRefresh bool, concurrency int, ext map[string]interface{}) ([]model.SearchResult, error) {
	...
	if !forceRefresh {
		...
		if esCacheService != nil && elasticsearch.IsEnabled() {
			results, hit, err := esCacheService.Get(cacheKey)
			if err == nil && hit {
				fmt.Printf("✅ [Plugin:%s] ES缓存命中 | 结果数: %d\n", keyword, len(results))
				// 记录搜索历史
				go esCacheService.RecordSearchHistory(keyword)
				return results, nil
			}
		...
		if cacheInitialized && config.AppConfig.CacheEnabled {
			...
			if enhancedTwoLevelCache != nil {
				...
				if err == nil && hit {
					...
					if err := enhancedTwoLevelCache.GetSerializer().Deserialize(data, &results); err == nil {
						// 返回缓存数据
						fmt.Printf("✅ [Plugin:%s] 本地缓存命中 | 结果数: %d\n", keyword, len(results))
						// 异步同步到ES
						if esCacheService != nil && elasticsearch.IsEnabled() {
							go esCacheService.Set(cacheKey, keyword, nil, plugins, "plugin", results)
						}
						return results, nil
					} else {
	...
	fmt.Printf("🔍 [Plugin:%s] 实时搜索完成 | 结果数: %d\n", keyword, len(allResults))
	...
	if len(allResults) > 0 {
		...
		if esCacheService != nil && elasticsearch.IsEnabled() {
			go func(res []model.SearchResult, kw string, key string) {
				if err := esCacheService.Set(key, kw, nil, plugins, "plugin", res); err != nil {
					fmt.Printf("❌ [Plugin:%s] ES缓存写入失败: %v\n", kw, err)
				} else {
					fmt.Printf("💾 [Plugin:%s] ES缓存写入成功 | 结果数: %d\n", kw, len(res))
				}
				// 记录搜索历史
				esCacheService.RecordSearchHistory(kw)
