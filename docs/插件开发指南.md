...
```bash
# QQPD 插件管理页面
GET  http://localhost:8889/qqpd/user123

# QQPD 插件配置接口
POST http://localhost:8889/qqpd/user123
...
# 自定义插件接口
GET  http://localhost:8889/myplugin/status
POST http://localhost:8889/myplugin/config
GET  http://localhost:8889/myplugin/resource123
```
...
```bash
# 使用API测试插件
curl "http://localhost:8889/api/search?kw=测试&plugins=myplugin"
```
